...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-brand-ink/70 backdrop-blur-md border-b border-white/10">
      ...
      {/* Mobile drawer */}
      {mobileMenuOpen && (
        <div id="mobile-nav" className="lg:hidden border-t border-white/10 bg-brand-ink px-4 pt-2 pb-6 shadow-xl">
              ...
              item.hasDropdown ? (
                <div key={item.label}>
                  <button
                    ...
                    className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm font-semibold text-gray-200 hover:bg-white/5 hover:text-brand-yellow"
                  >
                    <span>{item.label}</span>
                    <svg
                      className={`w-3.5 h-3.5 transition-transform duration-200 ${
                  ...
                  {mobileProductsOpen && (
                    <div id="mobile-products-submenu" className="mt-1 flex flex-col gap-0.5 pl-4">
                      {item.children?.map((subItem) => (
                        <Link
                          key={subItem.label}
                          href={subItem.href}
                          ...
                          className="rounded-lg px-3 py-2 text-sm text-gray-300 hover:bg-white/5 hover:text-brand-yellow"
                        >
                          {subItem.label}
                        </Link>
                      ))}
